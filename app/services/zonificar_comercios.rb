class ZonificarComercios
  ...
  def procesar
    ...
    zonas.each do |zona|
      ...
      Zona.find_or_create_by!(descripcion: zona[:nombre], ciudad_id: @ciudad_id)
  ...
  def obtener_zonas_elegibles
    zonas = Comercio.where(ciudad_id: @ciudad_id)
                    ...
                    .map do |zona_nombre, coordenadas|
      { nombre: zona_nombre,
        comercios: coordenadas.split(', ').reject(&:empty?) }
    end
  ...
  def generar_poligono(coordenadas)
    ...
    coordenadas.flatten!
    coordenadas.push(coordenadas[0], coordenadas[1]) unless [coordenadas[-2],
                                                             coordenadas[-1]] == [coordenadas[0], coordenadas[1]]
